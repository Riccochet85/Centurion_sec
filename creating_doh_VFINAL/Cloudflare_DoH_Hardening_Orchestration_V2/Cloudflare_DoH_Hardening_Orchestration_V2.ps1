...
Deployment: This script self-registers via Task Scheduler using SYSTEM privileges.
#>
...
if (-not $adapter) {
    Write-Log "WARNING: No active adapter detected. Attempting fallback selection."
    $adapter = Get-DnsClient | Select-Object -First 1
}
...
Write-Log "Registering scheduled task for persistent enforcement."
...
try {
    ...
    Write-Log ("ERROR creating scheduled task: {0}" -f $_.Exception.Message)
}
